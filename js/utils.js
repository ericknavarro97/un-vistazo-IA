...
let detectSmile = document.getElementById('smile');
...
function startVideoProcessing() {
  ...
  smileClassifier = new cv.CascadeClassifier();
...
function processVideo() {
  ...
  if (detectFace.checked) {
    ...
    if (detectEye.checked) {
      ...
      if (detectSmile.checked) {
        let smileVect = new cv.RectVector();
        let smileMat = new cv.Mat();
        cv.pyrDown(grayMat, smileMat);
        size = smileMat.size();
        smileClassifier.detectMultiScale(smileMat, smileVect);
        for (let i = 0; i < smileVect.size(); i++) {
          let smile = smileVect.get(i);
          smiles.push(new cv.Rect(smile.x, smile.y, smile.width, smile.height));
        }
        smileMat.delete();
        smileVect.delete();
      }
  ...
  drawResults(canvasOutputCtx, smiles, 'blue', size);
  stats.end();
  requestAnimationFrame(processVideo);
}
